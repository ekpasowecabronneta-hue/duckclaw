...
def serve(
    ...
    if pm2:
        ...
        # Propagate LangSmith + LLM vars if set
        for key in (
        ...
        config_content = f"""/**
 * PM2 config for DuckClaw LangGraph API server (generated by duckops serve --pm2).
...
module.exports = {{
  apps: [
    {{
      name: "{name}",
      script: "{python_path}",
      args: "-m duckclaw.agents.graph_server --host {host} --port {port}",
      cwd: "{effective_cwd}",
      interpreter: "none",
      autorestart: true,
      watch: false,
...
"""
        ...
        if existing.returncode == 0 and existing.stdout.strip() not in ("", "[]"):
            ...
            print(f"🚀  PM2: {name} iniciado.", flush=True)

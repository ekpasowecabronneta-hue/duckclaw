...
## Preguntas de ejemplo (OHLCV / IBKR)

Uso manual (Telegram u otra UI): el agente debe llamar `fetch_market_data` con el ticker y timeframe adecuados; **intradía típica** (`1m`…`1h` según `IBKR_REALTIME_TIMEFRAMES`) va al **HTTP** `IBKR_MARKET_DATA_URL` (lake + fallback IB en el VPS si aplica); **histórico lake** (`1d`, `1w`, `1M`, `moc` si están solo en histórico) va por **SSH** al lake si está configurado.

- «Trae velas de SPY en 1h, últimos 7 días y confirma cuántas filas quedaron en `quant_core.ohlcv_data`.» → `fetch_market_data` (HTTP IBKR).
- «Descarga OHLCV 15m de USO con 10 días de historia.» → HTTP IBKR.
- «Mete en la base el diario de NVDA, 180 días.» → rama **lake** si `1d` ∈ `CAPADONNA_HISTORICAL_TIMEFRAMES` y no choca con realtime; si solo quieres mirar sin persistir, `fetch_lake_ohlcv`.
- «¿Último cierre de QQQ en gráfico horario? Usa datos guardados o baja si faltan.» → `fetch_market_data` + `read_sql` si hace falta evidencia.
- «Comprueba 1d de GLD solo leyendo el lake, sin DuckDB.» → `fetch_lake_ohlcv` (no mezclar con `CAPADONNA_OFFLINE` de rutas HTTP).

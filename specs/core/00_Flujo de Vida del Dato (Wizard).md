...
| `duckops init` | Wizard Sovereign v2.0 (TUI `prompt_toolkit`, borrador + confirmación). |
| `duckops init --classic` | Wizard clásico (Rich, `scripts/duckclaw_setup_wizard.py`). |
...
├── pyproject.toml
└── duckops/
    ├── cli.py              # duckops init (Sovereign por defecto) + serve/deploy/audit
    ├── sovereign/        # Wizard v2.0 (draft, materialize, ui)
    └── commands/
        ├── init.py         # duckops init — Sovereign; --classic → wizard legacy

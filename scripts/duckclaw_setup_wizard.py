...
def _ensure_shared_import_path(repo_root: Path) -> Path:
    rr = Path(repo_root).resolve()
    for p in (rr, rr / "packages" / "shared" / "src"):
        sp = str(p)
...
def _repo_dotenv_immutable(repo_root: Path) -> bool:
    try:
        ...
        return False
    return is_repo_dotenv_immutable(_ensure_shared_import_path(repo_root))


def _env_persist_target_path(repo_root: Path) -> Path:
    """Ruta del archivo donde el wizard fusiona variables (``.env`` o propuesta si inmutable)."""
    rr = _ensure_shared_import_path(repo_root)
    try:
        from duckclaw.dotenv_immutable import is_repo_dotenv_immutable, proposed_env_path
    except ImportError:
        return rr / ".env"
    if is_repo_dotenv_immutable(rr):
        return proposed_env_path(rr)
    return rr / ".env"
...
def _edit_gateway_service(
    ...
    *,
    ...
    while True:
        ...
        _persist = _env_persist_target_path(repo_root)
        _where = "Propuesta" if _repo_dotenv_immutable(repo_root) else ".env"
        console.print(
            f"[green]✓[/] {_where} actualizada en [dim]{_persist}[/]: "
            f"DUCKCLAW_DB_PATH={db_path}, REDIS_URL=[dim]...[/], "
...
def _edit_service_settings(
    ...
    if provider == "pm2":
        ...
        cwd = str(repo_root)
        _tok_line = (
            " * Token: TELEGRAM_BOT_TOKEN desde .env del repo; si hay .env.immutable, el wizard fusiona propuestas en config/dotenv_wizard_proposed.env."
            if _repo_dotenv_immutable(repo_root)
            else " * Token: guardado en .env (auto-cargado por el bot al iniciar)."
        )
        config_content = f"""/**
...
"""
        ...
        _pt = _env_persist_target_path(repo_root)
        _lbl = "Propuesta fusionada en" if _repo_dotenv_immutable(repo_root) else "Variables guardadas en"
        console.print(f"[green]✓[/] {_lbl}: [dim]{_pt}[/]")
...
def _write_env_file(repo_root: Path, key: str, value: str) -> None:
    ...
    rr = _ensure_shared_import_path(repo_root)
    try:
        from duckclaw.dotenv_immutable import is_repo_dotenv_immutable, merge_proposed_env_file
    except ImportError:
        is_repo_dotenv_immutable = lambda _r: False  # noqa: E731
        merge_proposed_env_file = None  # type: ignore[assignment,misc]
    if key == "DUCKCLAW_DB_PATH":
        batch = {"DUCKCLAW_DB_PATH": value, "DUCKDB_PATH": value}
        if merge_proposed_env_file is not None and is_repo_dotenv_immutable(rr):
            merge_proposed_env_file(rr, batch)
            return
        _write_env_file_raw(rr, "DUCKDB_PATH", value)
        _write_env_file_raw(rr, "DUCKCLAW_DB_PATH", value)
        return
    if merge_proposed_env_file is not None and is_repo_dotenv_immutable(rr):
        merge_proposed_env_file(rr, {key: value})
        return
    _write_env_file_raw(rr, key, value)


def _write_env_file_raw(repo_root: Path, key: str, value: str) -> None:
    """Escribe una clave en ``.env`` sin comprobar inmutabilidad (uso interno)."""
...
def _run_section(
    ...
    if section_id == "token":
        _dot_immutable = _repo_dotenv_immutable(repo_root)
        _proposed_hint = (
            "\n[dim]La raíz `.env` es inmutable aquí: al guardar, el wizard fusiona en "
        ...
        console.print(
            Panel(
                f"Origen del token de Telegram.{_proposed_hint}",
        ...
        if token_from_env:
            ...
            if use_env:
                ...
                if state.get("token"):
                    _save_q = (
                        "¿Escribir TELEGRAM_BOT_TOKEN en config/dotenv_wizard_proposed.env "
                        "(`.env` raíz inmutable)?"
                        if _dot_immutable
                        else "¿Sobrescribir TELEGRAM_BOT_TOKEN en .env con este valor?"
                    )
            ...
            _paste_hint = (
                "[dim]Pégalo aquí o añádelo manualmente (p. ej. a `.env` o copiando desde "
                "`config/dotenv_wizard_proposed.env`) y vuelve a esta sección (a=anterior).[/]"
                if _dot_immutable
                else "[dim]Pégalo aquí o añádelo al archivo .env y vuelve a esta sección (a=anterior).[/]"
            )
            console.print(
            ...
            if state.get("token"):
                _save_q2 = (
                    "¿Guardar TELEGRAM_BOT_TOKEN en config/dotenv_wizard_proposed.env?"
                    if _dot_immutable
                    else "¿Guardar TELEGRAM_BOT_TOKEN en .env?"
                )
                save_e, nav_s = _confirm_with_nav(
                    console,
                    _save_q2,
